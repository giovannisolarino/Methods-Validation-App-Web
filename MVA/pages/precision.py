...
def intra_day():
    ...
    if days is not None:
        ...
        CV_global.to_json('./.nicegui/precision_intra.json')
...
def inter_day():
    ...
    if days is not None and days != 1:
        ...
        CV_inter.to_json('./.nicegui/precision_inter.json')
...
def handle_warning(e: events.ValueChangeEventArguments):
    if e.value <= 2:
        ui.notify('Unable to perform heteroscedasticity testing with two or less curves each day, the OLS method will be chosen as default', 
...
def precision():
    ...
    try:
        df = df = pd.read_json(app.storage.user['df'])
        means = means_data(df)
